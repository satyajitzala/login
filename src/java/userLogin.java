        ...
        try (PrintWriter out = response.getWriter()) {
           ...
           // out.println("<h1>Servlet userLogin at " + request.getContextPath() + "</h1>");
            ...
            try{
                ...
                Connection con =DriverManager.getConnection("jdbc:mysql://localhost:3306/registr","root","");
                PreparedStatement st=(PreparedStatement)con.prepareStatement("select * from registration1 where Email=? and Password=?"); 

                st.setString(1, email);
                st.setString(2, password);
                ResultSet rs=st.executeQuery();
 
                out.println("<h1>please enter username</h1>");
                if(rs.next()){
